...
/// <summary>
/// The class for the UI panel example using coroutine tweening, but applied with easing functions.
/// </summary>
public sealed class UIPanelExampleCoroutineTweenEasing : UIPanelExampleBase
{
    /// <summary>
    ...
    /// Default constructor.
    /// </summary>
    public UIPanelExampleCoroutineTweenEasing()
    {
    ...
    public override bool Show()
    {
        ...
        Coroutine coroutine = StartCoroutine(CoroutineShow(m_duration, Kryz.Tweening.EasingFunctions.OutExpo, () => { m_tweeningCoroutines.Clear(); }));
        return true;
    }
    ...
    /// <param name="onDone">The action to call when the tween is done.</param>
    /// <returns>IEnumerator of Coroutine.</returns>
    private IEnumerator CoroutineHide(float duration, Func<float, float> easing = null, Action onDone = null)
    {
    ...
    /// <param name="onDone">The action to call when the tween is done.</param>
    /// <returns>IEnumerator of Coroutine.</returns>
    private IEnumerator CoroutineFadeTween(float duration, float targetAlpha, Func<float, float> easing = null, Action onDone = null)
    {
        ...
        while (elapsedTime < duration)
        {
            ...
            float t = easing != null ? easing(elapsedTime / duration) : elapsedTime / duration;
    ...
    /// <param name="onDone">The action to call when the tween is done.</param>
    /// <returns>IEnumerator of Coroutine.</returns>
    private IEnumerator CoroutineMoveTween(Vector3 targetLocalPosition, float duration, Func<float, float> easing = null, Action onDone = null)
    {
        ...
        while (elapsedTime < duration)
        {
            elapsedTime += Time.deltaTime;
            float t = easing != null ? easing(elapsedTime / duration) : elapsedTime / duration;
            Vector3 position = Vector3.Lerp(startPosition, targetLocalPosition, t);
